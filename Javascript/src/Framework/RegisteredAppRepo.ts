...
interface AppUIBoolFunc {
    (configObj : AppUIConfig) : boolean
}
...
export class RegisteredApplicationRepository {
    ...
    public SetUIConfig(key:string, config : AppUIConfig) : void {
        if(!this.HasBeenRegistered(key)) {throw "App " + key + " has not been registered."}
    ...
    public GetAppsWhereUI( func :AppUIBoolFunc) : string[] {
        const appList = [];
        for(let property in this.RegisteredAppList) {
